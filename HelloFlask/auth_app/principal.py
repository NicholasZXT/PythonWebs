...
# ------------------------------------------------------------------------------------
"""
写在最前面的使用总结，Flask-Principal中，开发者需要关心的几个对象或者方法如下：
1. @Principal.identity_loader：此装饰器用于注册 **每次请求前** 获取用户身份的（无参）回调函数，用户身份使用 Identity 对象表示。
   **从源码逻辑来看，如果初始化Principal对象时设置了use_sessions=False，则必须要设置一个获取用户身份的回调函数，否则就一直是匿名用户状态**；
2. @Principal.identity_saver：此装饰器用于持久化已经获取的用户身份信息，供后续使用。
   注意，它需要和上面的 identity_loader 配合使用，两个函数中需要设置共用的 Identity 存放地点，一个存一个取。
默认下，Principal 会分别注册一个 session_identity_loader() 和 session_identity_saver()，
在Flask的全局 session 对象中存放 Identity 信息，实现一个简单的 跨请求保存用户身份。
3. 信号量 identity_loaded 对象和它的 connect() 装饰器：此装饰器用于注册一个回调函数，在用户身份Identity **加载之后** 的执行一些自定义操作。
   **一般会在这里实现 根据用户身份（Identity）查询用户所持有权限（Needs）的逻辑，然后存入 Identity.provides 里面**；
4. 信号量 identity_changed 对象和它的 .send() 方法：此方法用于通知Principal，用户身份有变更。
   **一般会在登录视图函数里调用此方法**
5. @Permission.require 装饰器：此装饰器用于保护需要校验权限的视图函数
6. Permission.can() 方法：此方法用于开发者 在视图函数中 进行权限校验时使用，和上面的装饰器 可以二选一，方便开发者使用
注意：
上面 @Principal.identity_loader 装饰器注册的函数是用来 获取 用户Identity身份的；
而 @identity_loaded.connect 装饰器注册的函数是用来根据 用户Identity身份 加载其他信息的，两者有先后顺序。
...
# 此信号量的 .send() 方法 **需要开发者来调用**，因为用户身份变更的逻辑要开发者来实现，一般有下面两种情况：
...
# 开发者在自定义用户身份加载时的操作时，需要使用此信号量的 @identity_loaded.connect 装饰器注册一个接收此信号量之后的回调函数，
# 常见的一个操作是，在回调函数里对用户所持有的角色/权限进行查询加载（如下面文档的例子里那样）。
# 使用 connect 装饰器方法注册的 回调函数，必须接受两个参数：第1个位置参数sender是Flask对象，一般不用管；第2个参数就是已获取的 Identity 对象
...
# Need 是某种操作权限的描述，它是最细粒度的权限对象，这里的最细粒度指的是具体到每一行记录的权限，比如 某用户可以编辑某个blog。
# 作者使用了元组作为表示 Need 的数据结构，并且预定义了 Need 和 ItemNeed 两种常见场景的权限表示：
# 1. 一种是像下面Need一样，只需要记录两个信息的权限对象，表示Identity需要属于该角色（类似用户组）才能通过权限校验：
#    method记录的是权限的分组类型，比如下面的 UserNeed, RoleNeed 等 —— 个人觉得method这个命名不太恰当
#    value就是该类型中具体某个角色，比如 RoleNeed 下的 ('role', 'admin') 标识 admin 角色权限
# 2. 另一种是下面 ItemNeed 一样，需要3个元素的元组描述，表示object级别的操作权限，也就是Identity对 某个表中 某行记录 进行CRUD操作的权限
...
# provides 字段记录的是一系列的 Need set，表示该用户持有的权限，一般在信号量 identity_loaded 的 connect() 方法里注册的hook函数里设置
...
# 返回时，它会持有当前的 Permission 对象，然后从Flask g 全局对象中获取当前请求的用户对象 Identity —— 因此它必须在 Flask上下文环境中才能使用
...
class IdentityContext(object):
    ...
    def __call__(self, f):
        ...
        def _decorated(*args, **kw):
            # 使用 with 语法，调用下面的 __enter__ 执行权限校验
            with self:
                rv = f(*args, **kw)
            return rv
        return _decorated
    ...
    def __enter__(self):
        ...
        if not self.can():  # 权限校验不通过时，准备抛出异常
            # 如果设置了要抛出的 http_exception，则调用 Flask 提供的 abort 方法抛出异常
            if self.http_exception:
                abort(self.http_exception, self.permission)
            # 否则抛出默认异常——这是个 RuntimeError，感觉这个默认异常的类型不是很合适
...
class Permission(object):
    ...
    # 用于装饰视图函数的方法  ------------------------------------------------------- KEY
    # 注意，require方法本身不是装饰器，它返回的 IdentityContext 对象的 __call__ 方法才是装饰器，
    # 所以使用的时候，require()后面的括号也是需要的
...
# 下面两个默认的 identity_loader/identity_save 方法，使用了 Flask 全局对象 session 来存取用户身份，而不是通过 g，
# 实现了一个简单的 跨请求保持用户身份 的功能
...
class Principal(object):
    ...
    def init_app(self, app):
        ...
        # 该方法里会依次调用 @Principal.identity_loader 装饰器注册的hook函数，获取用户身份的Identity对象
        # 然后再使用信号量 identity_loader.send() 方法，调用用户使用 @identity_loader.connect 装饰器注册的 hook 函数
        ...
        # ----------------------------------------------------------------------------------------------
        # 需要注意的地方：这里的回调函数即使更新了 g.identity，每次请求也会被上面的 self._on_before_request() 再次更新，
        # 因此如果需要跨请求保存用户身份，就需要在 self.identity_loaders 里注册一个加载用户身份的回调函数，
        # 否则就会一直是 g.identity = AnonymousIdentity 的状态
        # ----------------------------------------------------------------------------------------------
    ...
    def set_identity(self, identity):
        ...
        # 虽然上面的方法会向 g.identity 里存放一次，但是 g.identity 只在本次请求中有效；
        # 下面再调用自定义的 saver 存放一次，可以存放在其他地方，实现跨请求的用户身份存储 -------------------------- KEY
    ...
    def _set_thread_identity(self, identity):
        # 在这里向Flask全局对象 g 中存入的获取到的用户身份Identity，注意，这里存放的用户Identity只在当前请求中有效，不能跨请求使用
        g.identity = identity
    ...
    def _on_before_request(self):
        if self._is_static_route():
            return
        # 这里在每次请求调用视图函数之前都会将 g.identity 重置为 AnonymousIdentity -------------------------- KEY
